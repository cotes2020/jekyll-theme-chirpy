...
function _parseDotenvFile(filePath, override) {
 ...
 */
async function generateEmbedding(env, text, options = {}) {
    const surface = parseGenerativeSurfaceFromEnv(env);
    ...
    if (surface === 'vertex') {
        ...
        try {
            ...
            throw new Error(`Vertex 임베딩 파싱 실패: ${raw.slice(0, 300)}`);
        }
        if (!res.ok)
            throw new Error(data.error?.message || `Vertex 임베딩 HTTP ${res.status}`);
        const values = data.predictions?.[0]?.embeddings?.values;
        if (!Array.isArray(values) || values.length === 0)
            throw new Error('Vertex 임베딩 응답 비어있음');
        return values;
    }
    // AI Studio
    const apiKey = env.GEMINI_API_KEY?.trim();
    if (!apiKey)
        throw new Error('AI Studio 임베딩: GEMINI_API_KEY가 필요합니다.');
    ...
    try {
        ...
        throw new Error(`AI Studio 임베딩 파싱 실패: ${raw.slice(0, 300)}`);
    }
    if (!res.ok)
        throw new Error(data.error?.message || `AI Studio 임베딩 HTTP ${res.status}`);
    const values = data.embedding?.values;
    if (!Array.isArray(values) || values.length === 0)
        throw new Error('AI Studio 임베딩 응답 비어있음');
    return values;
}
// ─── Claude CLI 프로바이더 ─────────────────────────────────────────────────
