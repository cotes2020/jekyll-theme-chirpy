...
- Tauri가 주입: `window.__KARMOLAB_DESKTOP__ === true` (웹 브라우저에는 없음), `window.__KARMOLAB_DESKTOP_BUILD__` 는 **`"debug"`** 또는 **`"release"`** (헤더에 **개발** / **배포** 배지 표시에 사용)
...
- 트레이: **왼쪽 클릭**으로 메뉴(창 보이기 / 브라우저에서 열기 / **페이지: 배포 (GitHub)** · **페이지: 로컬 (8899)** / **업데이트 확인…** / 종료). 웹 페이지가 안 떠도 트레이만으로 주소를 바꿀 수 있습니다. 선택은 앱 설정 폴더의 **`karmolab-origin.json`** 에 저장되어 다음 실행 시에도 유지됩니다(트레이에서 한 번 고른 뒤에만 파일이 생깁니다). Windows에서는 트레이 아이콘 **왼쪽 더블클릭**으로 메뉴 없이 창만 앞으로.
- **브라우저에서 열기**는 현재 WebView 주소(배포 또는 로컬)를 그대로 기본 브라우저로 엽니다.
