// @ts-nocheck
/** Discord embed 길이 제한·Cursor/git 요약 헬퍼 */

export function truncateEmbedField(str, max = 1000) {
...
export function truncateDiscordDescription(str, max = 4090) {
    if (str == null || str === '') return '';
    const t = String(str);
    if (t.length <= max) return t;
    return `${t.slice(0, max - 1)}…`;
...
export function hasGitWorkingChanges(g) {
...
export function formatCursorAcpRpcSummaryField(s) {
