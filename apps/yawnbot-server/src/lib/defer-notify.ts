...
export function buildDeferProgressEmbed(kind, s) {
...
export async function startDeferElapsedTicker(interaction, kind, extra = {}) {
    ...
    const run = async () => {
        ...
        try {
            ...
            if (code !== 50006) console.error('[defer ticker] editReply 실패:', e.message);
        }
    };
...
export async function notifyDeferCompletion(interaction, { ok, kind }) {
