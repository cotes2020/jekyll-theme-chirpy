(function() {
    ...
    const MILESTONES = {
        100: { msg: '100번! 겨우 시작이다냥~', mood: 'happy', achievement: 'pet_100' },
        1000: { msg: '1,000번! 아직 999,000번 남았다냥', mood: 'smug', achievement: 'pet_1000' },
        10000: { msg: '10,000번! 집사 가끔 대단하다냥', mood: 'happy', achievement: 'pet_10000' },
        100000: { msg: '100,000번!! 진짜로 하고 있었냥?!', mood: 'shock', achievement: 'pet_100000' },
        500000: { msg: '500,000번!!! 반이다냥... 설마 진심이냥?!', mood: 'love', achievement: 'pet_500000' },
    ...
    Toolbox.register({
        ...
        tabs: [{ id: 'app', label: '쓰다듬기', build: function(container) {
            container.innerHTML = `
                <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:380px; gap:16px; text-align:center; position:relative; overflow:hidden;">
                    <div style="font-size:14px; color:var(--text-secondary);">🐱 고양이 머리를 무한히 쓰다듬어주세요냥</div>
            ...
            `;
            const petArea = container.querySelector('#petArea');
            const countLabel = container.querySelector('#petCount');
            const milestone = container.querySelector('#petMilestone');
            let count = Toolbox.getProgress(PROGRESS_KEY);
            countLabel.textContent = count.toLocaleString();

            Mdd.setMood('love'); Mdd.say('쓰다듬어달라냥~');
            ...
            petArea.addEventListener('mousemove', () => {
                ...
                count = Toolbox.incrementProgress(PROGRESS_KEY);
                countLabel.textContent = count.toLocaleString();
            ...
            function showMarriagePopup() {
                Mdd.setMood('love'); Mdd.say('결혼합시다냥!!! 💖💍');
                ...
                overlay.innerHTML = `
                    ...
                    <div style="font-size:32px; font-weight:bold; margin-top:20px;">검의 서약: 결혼합시다냥!</div>
                    <div style="font-size:14px; margin-top:10px; color:pink;">(주의: 이 창은 영원히 닫히지 않습니다냥 😼)</div>
                `;
