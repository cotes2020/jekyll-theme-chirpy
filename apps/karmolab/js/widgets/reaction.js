(function() {
    Toolbox.register({
        ...
        tabs: [{ id: 'app', label: '반응속도', build: function(container) {
            ...
            const icon = container.querySelector('#reactionIcon');
            const text = container.querySelector('#reactionText');
            const sub = container.querySelector('#reactionSub');
            const resultsEl = container.querySelector('#reactionResults');
            const historyEl = container.querySelector('#reactionHistory');
            const bestEl = container.querySelector('#reactionBest');

            const bestTime = Toolbox.getProgress('reaction_best');
            if (bestTime) bestEl.textContent = `최고 기록: ${bestTime}ms`;
            ...
            box.onclick = () => {
                switch (state) {
                    ...
                    case STATES.GREEN: {
                        ...
                        resultsEl.textContent = `평균: ${avg}ms · ${results.length}회 시도`;
                        ...
                        const currentBest = Toolbox.getProgress('reaction_best');
                        ...
                        if (reactionTime < 150) Toolbox.completeAchievement('reaction_150', { title: '번개 반응 150ms' });
                        Mdd.addAffection(1);
                        break;
                    }
                }
