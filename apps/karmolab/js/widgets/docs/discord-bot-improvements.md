...
- [x] **긴 플레이리스트 요약** — `/music play` 플레이리스트 완료 메시지에 목록 총곡·추가·URL 스킵 한 줄 요약
- [x] **실패 곡 사용자 안내** — `playNext` 재생 실패 시 로그 + 슬래시 친 텍스트 채널에 **쿨다운(약 15초)** 짧은 한 줄
...
| 구역 | 메모 |
|------|------|
| **DJ 권한 (옵션)** | 역할 기반 `/music skip`·`stop`·`remove` 등 — 당분간 보류 |
