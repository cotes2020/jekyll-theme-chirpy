...
- [ ] **지금 재생 표시** — `/nowplaying` 추가 또는 `/queue` 상단에 “재생 중” 한 블록 (제목·URL·가능하면 길이)
- [ ] **상태 보관** — `GuildMusicState`에 `currentTrack` 등 재생 중 메타(큐에서 `shift` 된 트랙과 동기화)
...
- [ ] **`/queue` 출력** — 2000자 넘으면 여러 메시지·첫/끝 N곡만·페이지 버튼(선택)
- [ ] **응답 정책 표** — 명령별 `ephemeral` vs 채널 공개 기준 정리 후 코드에 반영
