...
(function () {
    ...
    Toolbox.register({
        ...
        tabs: [
            { id: 'docs-intro', label: '소개', build: function (c) { Mdd.setMood('pointing'); Mdd.say('문서 페이지다냥!'); c.innerHTML = '<p class="docs-body" style="color:var(--text-secondary)">문서 불러오는 중...</p>'; loadDoc('intro.md').then(function (md) { renderMarkdown(c, md); }).catch(function () { renderMarkdown(c, '*문서를 불러오지 못했어요. 새로고침해 주세요.*'); }); } },
            { id: 'docs-planning', label: '기획', build: function (c) { Mdd.setMood('think'); Mdd.say('기획 문서를 읽어보라냥...'); c.innerHTML = '<p class="docs-body" style="color:var(--text-secondary)">문서 불러오는 중...</p>'; loadDoc('planning.md').then(function (md) { renderMarkdown(c, md); }).catch(function () { renderMarkdown(c, '*문서를 불러오지 못했어요. 새로고침해 주세요.*'); }); } },
            ...
            { id: 'docs-guide', label: '가이드', build: function (c) { Mdd.setMood('happy'); Mdd.say('사용법을 알려줄게냥~'); c.innerHTML = '<p class="docs-body" style="color:var(--text-secondary)">문서 불러오는 중...</p>'; loadDoc('guide.md').then(function (md) { renderMarkdown(c, md); }).catch(function () { renderMarkdown(c, '*문서를 불러오지 못했어요. 새로고침해 주세요.*'); }); } },
            { id: 'docs-server-guide', label: '서버·배포 가이드', build: function (c) { Mdd.setMood('think'); Mdd.say('서버 연결·배포는 여기서 확인하라냥!'); c.innerHTML = '<p class="docs-body" style="color:var(--text-secondary)">문서 불러오는 중...</p>'; loadDoc('server-guide.md').then(function (md) { renderMarkdown(c, md); }).catch(function () { renderMarkdown(c, '*문서를 불러오지 못했어요. 새로고침해 주세요.*'); }); } },
        ]
    });
})();
