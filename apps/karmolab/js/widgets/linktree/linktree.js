...
(function () {
    const ICONS = {
        ...
        twitter: '<path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-5.8 1.1 0 3-1.2 3-1.2z"/>',
    ...
    const CONFIG = {
        ...
        groups: [
            ...
            { group: '블로그·프로젝트', items: [
                { url: 'https://mascari4615.github.io/', label: '블로그', desc: '메모, 기록, 전략', icon: 'blog' },
            ]},
            { group: '기타', items: [
                /* 여기에 추가 링크를 넣으세요 */
            ]},
    ...
    function build(container) {
        ...
        function copyAndToast(text, btn) {
            function done() {
                if (typeof Toolbox !== 'undefined' && Toolbox.showToast) Toolbox.showToast('클립보드에 복사됨');
                var t = btn.title; btn.title = '복사됨!'; setTimeout(function () { btn.title = t; }, 2000);
            }
        ...
        emailBtn.onclick = function () { copyAndToast(email, emailBtn); };
        const linkBtn = document.createElement('button');
        linkBtn.type = 'button';
        linkBtn.className = 'linktree-intro-btn';
        linkBtn.textContent = '링크';
        linkBtn.title = '클립보드에 복사';
        linkBtn.onclick = function () { copyAndToast(linktreeUrl, linkBtn); };
        ...
        CONFIG.groups.forEach(function (g) {
            ...
            (g.items || []).forEach(function (link) {
                ...
                const body = link.desc
                    ? '<span class="linktree-card-body">' + link.label + '<div class="linktree-card-desc">' + link.desc + '</div></span>'
                    : '<span class="linktree-card-body">' + link.label + '</span>';
                let html = '<span class="linktree-card-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' + iconPath + '</svg></span>' + body;
                if (isEmail) {
                    html += '<button type="button" class="linktree-card-copy" title="클립보드에 복사"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' + ICONS.copy + '</svg></button>';
                }
                card.innerHTML = html;
                if (isEmail) {
                    ...
                    copyBtn.addEventListener('click', function (e) {
                        ...
                        function done() {
                            if (typeof Toolbox !== 'undefined' && Toolbox.showToast) Toolbox.showToast('클립보드에 복사됨');
                            copyBtn.title = '복사됨!';
                            setTimeout(function () { copyBtn.title = '클립보드에 복사'; }, 2000);
                        }
                        if (navigator.clipboard?.writeText) {
                            navigator.clipboard.writeText(text).then(done);
                        } else {
    ...
    Toolbox.register({
        ...
        tabs: [{ id: 'linktree-main', label: '링크', build: build }],
    });
})();
