    ...
    tabs: [
      {
        ...
        build: function (container: HTMLElement): void {
          ...
          type SandPixel = { x: number; y: number; color: string };
          const grid: SandPixel[] = [];
          const cols = 60;
          const rows = 60;
          ...
          for (let y = 0; y < 30; y++) {
            for (let x = 10; x < 50; x++) {
              if (Math.random() > 0.3)
                grid.push({ x, y, color: `hsl(${20 + Math.random() * 20}, 70%, 60%)` });
            }
          }
          ...
          function animate(): void {
            ...
            for (let i = grid.length - 1; i >= 0; i--) {
              ...
              c2d.fillRect(p.x * currentW, p.y * currentH, currentW - 1, currentH - 1);
            }
