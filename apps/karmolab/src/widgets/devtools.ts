...
(function () {
    ...
    function buildNotifyPayload(titleIn, bodyIn, soundSel, imageIn) {
        const title = (titleIn.value || '').trim() || 'KarmoLab';
        const body = (bodyIn.value || '').trim() || 'KarmoLab';
        const o = { title: title.slice(0, 120), body: body.slice(0, 2000) };
    ...
    function buildNotifySection(wrap) {
        ...
        const mkField = function (labelText, inner) {
        ...
        btn.addEventListener('click', function () {
            ...
            desktopInvoke('desktop_notify', payload)
                .then(function () {
