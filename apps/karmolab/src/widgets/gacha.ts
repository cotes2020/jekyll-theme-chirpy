    ...
    tabs: [
      {
        ...
        build: function (container: HTMLElement): void {
          ...
          container.innerHTML = `
            ...
            `;
          const btnEl = container.querySelector('#gachaBtn') as HTMLButtonElement | null;
          const logEl = container.querySelector('#gachaLog') as HTMLElement | null;
          if (!btnEl || !logEl) return;
          ...
          function simulate(): void {
            ...
            for (let i = 0; i < 100; i++) {
              ...
              if (match === 6) {
                log.innerHTML =
                  `<div style="color:var(--success); font-weight:bold; font-size:15px;">🎉대박! ${attempts.toLocaleString()}번 만에 로또 1등 당첨! 🎉</div>` +
                  log.innerHTML;
                isRunning = false;
                btn.textContent = '시뮬레이션 재시작';
            ...
            log.innerHTML =
              `<div style="color:var(--text-secondary)">🔄 ${attempts.toLocaleString()}회 시도 중... (당첨 확률 1/8,145,060)</div>` +
              log.innerHTML;

            if (log.children.length > 50 && log.lastChild) log.removeChild(log.lastChild);
