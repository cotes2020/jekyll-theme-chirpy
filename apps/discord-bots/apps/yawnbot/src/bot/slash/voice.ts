...
export async function handleVoiceJoin(ctx, interaction) {
  ...
  if (!target.isVoiceBased()) {
    await interaction.reply({ content: '음성 채널이 아닙니다.', ephemeral: true });
    return;
  }
  const botMember = interaction.guild.members.me;
  if (!botMember) {
    await interaction.reply({ content: '봇 멤버 정보를 불러올 수 없습니다.', ephemeral: true });
    return;
  }
  ...
  const result = await joinVoiceChannelSafe(target);
  if (!result.ok) {
...
export async function handleVoiceLeave(ctx, interaction) {
  ...
  if (!ok) {
    await interaction.reply({ content: '이 서버에서 음성 채널에 연결되어 있지 않습니다.', ephemeral: true });
    return;
  }
  await interaction.reply({ content: '음성 채널에서 나갔습니다.', ephemeral: true });
}
