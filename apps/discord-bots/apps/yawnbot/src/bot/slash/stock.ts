...
export async function handleStockList(ctx: BotContext, interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  for (const st of list) {
    const change =
      st.diff > 0
        ? gameData.getMessage('Stock_List_Change_Up', formatMoney(st.diff), st.pct)
        : st.diff < 0
          ? gameData.getMessage('Stock_List_Change_Down', formatMoney(st.diff), st.pct)
...
export async function handleMyStock(ctx: BotContext, interaction: ChatInputCommandInteraction, userId: string, userName: string): Promise<void> {
  ...
  if (p.entries.length === 0) {
    ...
    for (const e of p.entries) {
      ...
      embed.addFields({
        name: `${e.name} (${e.amount}주)`,
        value: gameData.getMessage('Stock_MyStock_Item', e.avgPrice, formatMoney(e.currentPrice), profitStr, formatMoney(Math.abs(e.profit)), e.profitPct),
      });
    }
    embed.setFooter({
      text: gameData.getMessage(
        'Stock_MyStock_Footer',
        formatMoney(p.totalInvested),
