...
- [DS - pythonds3 - 3. Analysis](#ds---pythonds3---3-analysis)
  ...
  - [3.3. Big-O Notation](#33-big-o-notation)
    ...
    - [Asymptotically constant time operation `O(1)`](#asymptotically-constant-time-operation-o1)
    - [`O(㏒N)` 二分查找算法](#on-二分查找算法)
    - [线性算法 linear time `O(n)`](#线性算法-linear-time-on)
    - [`O(NlogN`) 二分查找算法](#onlogn-二分查找算法)
    - [O($n^2$)](#on2)
...
- old pc
- other running progress

measure the time:
- measuring the 渐近的 `asymptotic complexity` of a program
- Big O Notation

sorting an array of size 1000 compared to an array of size 10

**how does the runtime of your program input grow?**
...
### Asymptotically constant time operation `O(1)`
