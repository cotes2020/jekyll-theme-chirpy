...
- [other](#other)
  - [Binary Search Tree(BST) 左小右大](#binary-search-treebst-左小右大)
...
- preorder and postorder traversals are common ways of visiting the positions of a tree, another approach is to traverse a tree so that we visit `all the positions at depth d` before we visit the `positions at depth d + 1.`
