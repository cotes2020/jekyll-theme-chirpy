...
Ниже пример dev.env , если будете пушить это дело в Github, в публичную репу, не забываем это дело добавить в .gitignore или убрать в приватный репозиторий. Поэтому в моем Github данных файлов нет. Короче создайте по аналогии env файлы в корне проекта.
```
DEBUG=1
...
Вроде бы все, но если что-то упустил, вы можете скачать шаблон с моего Github.
...
```
Ну все, теперь можно создавать приложения и писать логику, но это еще не все.
...
## Админка и стили
Если вы попробуйте перести в админку, то увидите, что стили Django не погружаются. Из-за чего это происходит? Во-первых, мы сказали Django от куда брать стили. Давайте настроим наши css файлы для нашего upload приложения. Скачиваем с официального сайта проект django и скопируем папку admin из папки contrib в static. И проверим повторно приложение. 
