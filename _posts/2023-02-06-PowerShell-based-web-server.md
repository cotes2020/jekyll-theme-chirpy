...
Setting up a web server with PowerShell is very simple.  In the case of this example, I'm assuming it will operate from within a container. The container I'm using is based on dotnet6 and PowerShell 7 running on a debian linux os.
...
In this example, it shows the URLPrefix with the magic `+` in the place of the IP.  The `+` in dotnet is the equivalent of `0.0.0.0` which will be important for a container.  Port `8080` was used instead of 80.  I initially used 80 but at some point, I encountered a port conflict.  I think this has something to do with how dotnet starts the listener (not really sure).  To get around this I selected `8080` instead.  There might be a special switch that can be disable the port 80 conflict, but I didn't see it.

The URL path is set to`/test`. The web service GetContext will respond to any request. In this example the code will only respond to the my specific path. `New-Object` was used to call the dotnet library and assign it to the variable `$listener`. One thing to note with the URL, a closing slash `/` is required by the HttpListener property. If one is a ending `/` is not used, PowerShell will throw the following error.
...
The `$listener` object has a number of interest properties, IsListening, Prefixes, and AuthenticationSchemes.  IsListening is boolean with a value of True. Under Prefix it will show the assigned path. Once the GetContext() is executed we can test the server by connecting to localhost address, [http://127.0.0.1:8080/test/](http://127.0.0.1:8080/test/){:target="_blank"}.

Once, all the URL's are added the service can be started, but calling Start().
Before the GetContext is executed, we can check the `$listener` object and see it's properties.
...
The `$context.Request` object also has a number of useful settings.  In this example RawUrl, and HttpMethod are used to identify the uri and method.
...
From the client perspective, when the web server was listening for a Context request. A web request can be performed with Invoke-WebRequest or just opening a web browsers to [http://127.0.0.1:8080/test/](http://127.0.0.1:8080/test/){:target="_blank"}.  When the web server completes it's transaction it will result in a new Guid string. The RawContent shows the header and GUID, but the content shows the data in Bytes. To convert the Bytes to a string, [System.Text.Encoding][encoding]{:target="_blank"} is used.
