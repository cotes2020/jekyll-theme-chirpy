...
\delta L & = \sum_i \frac{\partial L}{\partial \pmb{q}_{i}} \cdot \delta \pmb{q}_{i} + \sum_i \frac{\partial L}{\partial \dot{\pmb{q}}_{i}} \cdot \delta \dot{\pmb{q}}_{i} \\
 ...
 & = \frac{1}{\epsilon} \frac{d}{dt} \left( \sum_i \pmb{p}_{i} \cdot \pmb{Q} \right)
\end{align}
$$

If the system is symmetric under the transformation generated by $$\pmb{Q}$$, its action remains invariant, which implies the variation of its Lagrangian must be zero. It thus follows,
...
If the above is true for all $$\pmb{X} \in \mathbb{A}^4 \left( \mathbb{R} \right)$$ where $$\mathbb{A}^n \left( \mathbb{R} \right)$$ is the affine 4-dimensional space over the reals, we must have,
...
\frac{d}{dt} \sum_i m_i \pmb{R} \: \dot{\pmb{x}}_i + \frac{d}{dt} \left( \sum_i m_i \right) \pmb{R} \: \pmb{v} & = \pmb{0}
\end{align}
$$

As $$\pmb{R}$$ is a linear transformation and $$\dot{R} = \pmb{O}$$,
