...
- Whether a developer, client or general consumer of the LLM, it is important to understand the implications of how `this vulnerability could reflect risks within the LLM application when interacting with a non-proprietary LLM`.
...
- While LLMs can produce creative and informative content, they can also generate content that is factually incorrect, inappropriate or unsafe. This is referred to as `hallucination` or `confabulation` and can result in misinformation, miscommunication, legal issues, and reputational damage.
