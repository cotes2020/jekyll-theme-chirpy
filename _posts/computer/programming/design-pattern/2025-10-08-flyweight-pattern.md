...
last_modified_at: 2025-10-09. 08:07 # E +예제: '워드 프로세서', ~정리: '오타 수정 및 표현 다듬기'
---
...
이런 문제를 해결하기 위해 경량 패턴은 객체 데이터를 두 종류로 나눈다. 먼저 모든 객체의 데이터 값이 같아서 공유할 수 있는 데이터를 모은다. 이런 데이터를 GoF는 **고유 상태**라고 했다. (책에서는 **자유 문맥** 상태라고 한다.) 예제에서는 나무 형태나 텍스처가 이에 해당한다.  
...
class Character {
private:
    // 글자의 모양 데이터 (메모리를 많이 차지함)
    Font font_;
    int size_;
    Color color_;
    GlyphData glyph_; // 'A'를 그리는 방법 등
...
워드 프로세서에서 각 글자 하나하나를 폰트, 크기, 색상, 스타일, 위치 등의 모든 정보를 가진 독립적인 객체로 만든다면 어떻게 될까?  
...
문서에 똑같은 'A' 글자가 1,000번 나온다면, 'A'의 모양을 정의하는 무거운 데이터 (`font_`, `glyph_` 등)가 메모리에 1,000번 중복으로 로드되게 된다. 이는 엄청난 메모리 낭비이다.  

경량 패턴은 이 문제를 해결하기 위해 글자 객체의 데이터를 두 종류로 나눈다.  

- **고유 상태 (Intrinsic State)**: 여러 객체가 **공유할 수 있는** 불변(immutable) 데이터.
  - **예시**: 글자의 모양을 결정하는 데이터 (폰트, 크기, 색상, 스타일, 글자 코드 'A' 등). 'Arial 12pt A'는 문서 어디에 있든 모양이 같다.
  - 이 공유 데이터를 **Flyweight 객체**라고 부른다.

- **외부 상태 (Extrinsic State)**: 각 **객체마다 고유해야 하는** 데이터. Flyweight 객체 외부에서 관리된다.
  - **예시**: 문서 내에서 글자의 **위치 (행, 열)**. 'A'가 첫 줄에 있을 수도, 마지막 줄에 있을 수도 있다.

워드 프로세서는 'Arial 12pt A'라는 Flyweight 객체를 메모리에 **단 하나만** 생성한다. 그리고 문서에 'Arial 12pt A'가 1,000번 등장하면, 1,000개의 다른 위치(외부 상태)에 이 동일한 Flyweight 객체를 참조하여 화면에 그리는 방식이다.  
...
몬스터들의 정보를 각 `MonoBehaviour`에 변수로 구현할 수 있다 (좋은 방법은 아니다). `monsterName`, `monsterDescription`, `hpMAX` 같은 값들은 보통 런타임 중에 변하지 않는 정보다. 만약 똑같은 몬스터를 여러 번 생성하는 경우, 이런 값들은 메모리에 중복되어 올라가게 되고, 이는 낭비가 될 수 있다.  
...
`monsterName`, `monsterDescription`, `hpMAX`처럼 공용으로 사용하는 데이터는 `MonsterData` 같은 스크립터블 오브젝트를 만들어 분리한다. 그리고 `Monster`는 직접 데이터를 가지고 있는 대신, 이 `MonsterData` 스크립터블 오브젝트를 참조하도록 수정한다.  

이렇게 되면 똑같은 몬스터를 몇 백, 몇 천 마리 생성하더라도 공용 데이터는 메모리에 스크립터블 오브젝트 하나만 올라가게 된다. (= 메모리 최적화)  
...
- **키워드:**
  ...
  - Intrinsic State (고유 상태), Extrinsic State (외부 상태)
    - Context-Free (자유 문맥)
