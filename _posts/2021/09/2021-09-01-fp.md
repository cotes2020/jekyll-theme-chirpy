...
可以使用不可变数据来避免这种情况，可以参考 [ImmutableJS 实战](https://www.notion.so/a60a508d75a1470080d65f5df2090d4c) 、ES6语法中的let/const与`Object.freeze()/Object.isFrozen()`
...
**偏函数应用定义**：partial application是指一种减少函数参数个数`Arity`的过程。
...
- 切割传参数的时空背景（时间、程式的不同区块），原本的方式需要在调用的时候立刻传入所有的参数，如果你的函数中有些参数待会才传入，可以考虑使用**currying**和**partial application**。
- 实现柯里化（见下一部分）
...
**Point-free**（又写成Pointfree，中文：无参数，无点），正式名称为：**tacit programming**，其中的**point**（点）指的就是函数的**parameter**（形式参数）。


**作用**：Pointfree透过隐藏**parameter** - **argument**（**形参**-**实参**对应），减少视觉上的干扰，上层操作不直接操作数据，只合成运算过程。
...
> 来自 [Haskell](https://hackage.haskell.org/package/base-4.8.1.0/docs/Data-Functor.html) 和 [Fantasy Land specification](https://github.com/fantasyland/fantasy-land)。
...
- B：map(...)传3个变数`value`, `index`,`array`
- A：parseInt(...)接收两个变数`string`,`radix`
