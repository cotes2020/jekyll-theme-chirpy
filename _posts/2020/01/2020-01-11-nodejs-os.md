---
title: "操作系统性能监控 - By NodeJS"
url: "2020-01-11-nodejs-os"
date: 2020-01-11
---
...
- 操作系统：提供了`os.platform()`和`os.type()`，可以用来识别操作系统平台。推荐使用: `os.platform()`
- 运行时间：
	- nodejs 运行时间：`process.uptime()`
	- 系统运行时间：`os.uptime()`
- 常用路径
	- 系统（windows/macos）空设备：`os.devNull`
	- 当前用户主路径：`os.homedir()`
	- 系统临时文件目录：`os.tmpdir()`
- 进程调度优先级
	- 查询指定进程优先级：`os.getPriority([pid])`
	- 修改指定/当前进程优先级：`os.setPriority([pid, ]priority)`
...
在 nodejs 中，直接调用`os.loadavg()`可以获得 1、5 和 15 分钟的平均负载，它和 unix 命令`uptime`返回值一样。
...
// 判断是否平均负载过高
function isHighLoad() {
    const cpuNum = os.cpus().length; // 获取当前电脑CPU内核的数量
...
**第一步**：封装`getCPUInfo()`，计算获取 cpu 花费的总时间与空闲模式花费的时间。
...
- [第三方扩展库：os-utils](https://www.npmjs.com/package/os-utils)
- [系统平均负载load average详解 转载](https://blog.51cto.com/u_15077533/4173309)
- [字节序（大小端）理解](https://blog.csdn.net/sunflower_della/article/details/90439935)
